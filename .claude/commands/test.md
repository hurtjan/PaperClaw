...
2. **Back up real DB.** Run:
   ```bash
   python3 scripts/test/test_helpers.py mkdir data/tmp/_test_backup
   python3 scripts/test/test_helpers.py cp data/db/papers.json data/tmp/_test_backup/
   python3 scripts/test/test_helpers.py cp data/db/authors.json data/tmp/_test_backup/
   python3 scripts/test/test_helpers.py cp data/db/contexts.json data/tmp/_test_backup/
   python3 scripts/test/test_helpers.py cp data/db/lit.duckdb data/tmp/_test_backup/
   python3 scripts/test/test_helpers.py cp data/db_history/manifest.jsonl data/tmp/_test_backup/
   python3 scripts/test/test_helpers.py truncate data/db_history/manifest.jsonl
   ```
...
4. **Initialize empty DB files** (use Bash with inline Python for cross-platform date handling):
   ```bash
   python3 -c "import json,datetime;d=datetime.date.today().isoformat();open('data/db/papers.json','w').write(json.dumps({'metadata':{'last_updated':d,'owned_count':0,'stub_count':0},'papers':{}}))"
   python3 -c "import json,datetime;d=datetime.date.today().isoformat();open('data/db/authors.json','w').write(json.dumps({'metadata':{'person_count':0,'institution_count':0,'processed_paper_count':0,'last_updated':d},'persons':{},'institutions':{},'processed_papers':[]}))"
   python3 -c "import json,datetime;d=datetime.date.today().isoformat();open('data/db/contexts.json','w').write(json.dumps({'generated':d,'owned_count':0,'total_papers':0,'owned_papers':[],'by_cited':{},'by_purpose':{},'citation_counts':{}}))"
   ```
...
5. **Copy** all PDFs from the first test subdir to `pdf-staging/` (do NOT move — originals must remain). Copy each PDF individually:
   ```bash
   python3 scripts/test/test_helpers.py cp pdf-staging/<first_test_dir>/<file>.pdf pdf-staging/
   ```
...
8. **Verify results:**
   ...
   - Run `python3 scripts/py.py scripts/build/check_db.py` — **FAIL Phase A** if exit code non-zero
   - Run `python3 scripts/py.py scripts/build/build_duckdb.py`
...
10. **Save Phase A snapshot:**
    ```bash
    python3 scripts/test/test_helpers.py mkdir data/tmp/_test_phase_a
    python3 scripts/test/test_helpers.py cp data/db/papers.json data/tmp/_test_phase_a/
    python3 scripts/test/test_helpers.py cp data/db/contexts.json data/tmp/_test_phase_a/
    ```
...
11. **Copy** all PDFs from the second test subdir to `pdf-staging/`. Copy each PDF individually:
    ```bash
    python3 scripts/test/test_helpers.py cp pdf-staging/<second_test_dir>/<file>.pdf pdf-staging/
    ```
...
14. **Verify results:**
    - Confirm `data/extractions/<id>.json` exists for each new paper ID
    - Run `python3 scripts/py.py scripts/build/check_db.py` — **FAIL Phase B** if exit code non-zero
    - Run `python3 scripts/py.py scripts/build/build_duckdb.py`
...
16. **Create merge source:**
    ```bash
    python3 scripts/test/test_helpers.py mkdir data/tmp/_test_merge_src/data/db
    python3 scripts/test/test_helpers.py cp data/db/papers.json data/tmp/_test_merge_src/data/db/
    python3 scripts/test/test_helpers.py cp data/db/contexts.json data/tmp/_test_merge_src/data/db/
    ```
...
18. **Run merge:**
    ```bash
    python3 scripts/py.py scripts/enrich/merge_db.py data/tmp/_test_merge_src --name test_corpus
    ```
...
21. **Verify manifest exists and has entries** (written during Phases A–C):
    ```bash
    python3 scripts/test/test_helpers.py wc data/db_history/manifest.jsonl
    ```
    **FAIL Phase D** if file is missing or has 0 lines.

22. **Verify patch files exist:**
    ```bash
    python3 scripts/test/test_helpers.py ls "data/db_history/patches/*.json"
    ```
    **FAIL Phase D** if no output (0 files).

23. **Display history:**
    ```bash
    python3 scripts/py.py scripts/build/rollback.py
    ```
    Record the number of entries shown.

24. **Preview rollback (dry run):**
    ```bash
    python3 scripts/py.py scripts/build/rollback.py --dry-run --last 1
    ```
    Verify output describes a change without modifying files.

25. **Apply rollback:**
    ```bash
    python3 scripts/py.py scripts/build/rollback.py --last 1
    ```
...
30. **Write merge plan** — write `data/tmp/duplicate_merge_plan.json`:
    ```bash
    python3 scripts/py.py scripts/test/write_merge_plan.py
    ```
    **FAIL Phase E** if exit code non-zero.

31. **Dry-run merge:**
    ```bash
    python3 scripts/py.py scripts/build/merge_duplicates.py --dry-run
    ```
    Verify output describes the expected merge (shows enrich/edges/refs summary). **FAIL Phase E** if exit code non-zero.

32. **Actual merge:**
    ```bash
    python3 scripts/py.py scripts/build/merge_duplicates.py
    ```
    **FAIL Phase E** if exit code non-zero.
...
35. **Run unit tests:**
    ```bash
    python3 scripts/py.py scripts/build/test_find_matches.py
    python3 scripts/py.py scripts/build/test_merge_duplicates.py
    ```
...
36. **Remove temp working dirs** (keep backup intact):
    ```bash
    python3 scripts/test/test_helpers.py rm data/tmp/_test_phase_a data/tmp/_test_merge_src
    ```

37. **Restore db_history state** (remove test patches, restore real manifest):
    ```bash
    python3 -c "from pathlib import Path; [f.unlink() for f in Path('data/db_history/patches').glob('*.json') if f.exists()]"
    python3 scripts/test/test_helpers.py cp data/tmp/_test_backup/manifest.jsonl data/db_history/manifest.jsonl
    ```
