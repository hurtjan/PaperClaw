...
Usage:
  ...
  python3 scripts/query/research.py missing <id>
      Papers in finding that are NOT in the database (candidates to add)
...
def load_db_papers():
    if duckdb is None or not DB_FILE.exists():
        return {}
    con = duckdb.connect(str(DB_FILE), read_only=True)
    rows = con.execute("SELECT paper_id, type FROM papers").fetchall()
    con.close()
    return {r[0]: {"type": r[1]} for r in rows}
...
def cmd_missing(fid, findings, db_papers):
    ...
    if not missing:
        print(f"All papers in '{fid}' are in the database.")
        return

    print(f"Papers in '{fid}' NOT in the database ({len(missing)} total):\n")
    for p in missing:
        authors = p.get("authors", "")
        year = p.get("year", "")
        title = p.get("title", "")
